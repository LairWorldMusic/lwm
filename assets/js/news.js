(function(){
  ...
  // Загружаем общие новости сайта из JSON файла
  fetch('assets/data/news.json', { cache: 'no-cache' })
    .then(r => r.json())
    .then(json => render(normalize(json)))
    .catch(() => {
      // fallback: показываем сообщение
      grid.innerHTML = '<p style="color: var(--muted); text-align: center; padding: 60px 20px; grid-column: 1/-1;">Новости скоро появятся</p>';
    });
})();
